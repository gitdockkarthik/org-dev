"""Storage abstraction for the Alert Analyser.
...
(default "postgres"):
  ...
  * PostgresBackend — durable, encrypted-at-rest config in the AgentConfig
    table (mirrors the existing routes_settings _upsert / load_config_from_db).
  * MemoryBackend   — process-local dict; non-persistent, handy for tests and
    DB-less runs.

Usage:
    init_storage("alert-analyser")       # once, on startup
    backend = get_backend()              # anywhere after init
    await backend.set("source_type", '"standalone"')
...
class PostgresBackend(StorageBackend):
    ...
    Secret keys are encrypted at rest. All DB errors are caught and logged —
    methods never raise.
    """
