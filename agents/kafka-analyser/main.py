                ...
                for c in enabled:
                    ...
                    try:
                        ...
                        if not _data_is_fresh:
                            if _age_minutes is not None:
                                logger.info(
                                    "Startup: existing topic structure for '%s' is %.0f min old (>90 min) -- refreshing",
                                    c["name"], _age_minutes
                                )
                            else:
                                logger.info("Startup: no existing topic structure data for '%s' -- fetching", c["name"])
                            try:
                                import time as _t2
...
@app.get("/runs")
async def get_all_runs(limit: int = 50, offset: int = 0, status: str | None = None, job_type: str | None = None) -> list:
    return await _jobs_module.get_runs(limit=limit, offset=offset, status=status, job_type=job_type)
