"""Self-contained job management for Alert Analyser.
Handles scheduled OpsGenie polling with overlap prevention,
...
from models import AlertJobRun, AlertJobSchedule
...
def register_job(job_id: str, name: str, description: str, handler) -> None:
    _jobs[job_id] = {"id": job_id, "name": name, "description": description, "handler": handler}
    logger.info("Job registered: %s", job_id)


async def _get_active_run(job_id: str) -> AlertJobRun | None:
    if SessionLocal is None:
        return None
    async with SessionLocal() as session:
        result = await session.execute(
            select(AlertJobRun).where(AlertJobRun.job_id == job_id, AlertJobRun.status == "running")
        )
        return result.scalar_one_or_none()


async def _create_run(job_id: str, triggered_by: str) -> AlertJobRun:
    async with SessionLocal() as session:
        run = AlertJobRun(
            job_id=job_id,
...
async def _update_run(run_id: int, **kwargs) -> None:
    if SessionLocal is None:
        return
    async with SessionLocal() as session:
        await session.execute(update(AlertJobRun).where(AlertJobRun.id == run_id).values(**kwargs))
        await session.commit()
...
async def _execute_job(job: dict, run: AlertJobRun) -> None:
...
async def load_schedules() -> int:
    ...
    async with SessionLocal() as session:
        result = await session.execute(select(AlertJobSchedule).where(AlertJobSchedule.enabled == True))
...
async def get_runs(job_id: str | None = None, limit: int = 50) -> list:
    if SessionLocal is None:
        return []
    async with SessionLocal() as session:
        q = select(AlertJobRun).order_by(desc(AlertJobRun.created_at)).limit(limit)
        if job_id:
            q = q.where(AlertJobRun.job_id == job_id)
...
async def get_schedules(job_id: str | None = None) -> list:
    if SessionLocal is None:
        return []
    async with SessionLocal() as session:
        q = select(AlertJobSchedule)
        if job_id:
            q = q.where(AlertJobSchedule.job_id == job_id)
        result = await session.execute(q)
...
async def create_schedule(job_id: str, cron_expression: str, enabled: bool = True) -> dict:
    ...
    async with SessionLocal() as session:
        sched = AlertJobSchedule(job_id=job_id, cron_expression=cron_expression,
...
async def update_schedule(schedule_id: int, cron_expression: str, enabled: bool) -> dict:
    if SessionLocal is None:
        return {"ok": False}
    async with SessionLocal() as session:
        await session.execute(update(AlertJobSchedule).where(AlertJobSchedule.id == schedule_id)
...
async def delete_schedule(schedule_id: int) -> dict:
    if SessionLocal is None:
        return {"ok": False}
    async with SessionLocal() as session:
        sched = await session.get(AlertJobSchedule, schedule_id)
        if sched:
