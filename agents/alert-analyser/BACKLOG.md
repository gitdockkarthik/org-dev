...
| 25 | Alert lifetime totals counter — alembic migration + verified versioning approach | 45 min | Corrected assumption during this work: alert-analyser's core tables (alert_reports, alert_job_runs) are created via Base.metadata.create_all(), but SOME schema additions (0005_incident_purge_audit.py, 0011_alert_escalation_log.py) DO go through the shared backend/migrations/ Alembic chain (mixed pattern, not create_all()-only as first assumed). New alert_lifetime_totals table added as proper migration 0041_alert_lifetime_totals.py (revises 0040). Table was created manually via SQL first during investigation and seeded with today's totals (228/184/44/0) before the Alembic convention was confirmed - reconciled via 'alembic stamp 0041' rather than dropping/recreating, preserving seed data. DB confirmed at 0041 (head), no drift. | 25 | 10/08/26 | 0.75 | Done |
